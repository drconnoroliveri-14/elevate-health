    ...
    <main className="min-h-screen font-sans overflow-x-hidden">
      ...
      <nav className="bg-teal-900 px-4 sm:px-6 py-3 sm:py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-3">
          {/* clamp() scales the logo down on narrow viewports without a breakpoint jump */}
          <img
            src="/logo.PNG"
            alt="Elevate Health"
            style={{ height: 'clamp(48px, 10vw, 80px)', width: 'auto', filter: 'brightness(0) invert(1)', flexShrink: 0 }}
          />
          {/* Single button — responsive padding & text size, no nowrap so it wraps if needed */}
          <a
            href={FULLSCRIPT_URL}
            target="_blank"
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 sm:px-6 max-w-4xl mx-auto w-full">
          ...
          {/* text-4xl on mobile (36px) prevents overflow on 320px screens */}
          ...
          {/* w-full on mobile so button never overflows its container */}
          <a
            href={FULLSCRIPT_URL}
            target="_blank"
            rel="noopener noreferrer"
